        ...
        <header className="bg-primary p-5 w-full flex items-center justify-between flex-between">
            <aside className="text-white flex justify-center items-center gap-4 w-full lg:w-1/4 lg:flex-row flex-col">
                <Link to="/" className="flex items-center gap-5">
                    <img className="w-11 object-contain" src="public/ciervologo.png" alt="logo ciervus" />
                    <p className="font-bold text-content lg:text-title">CIERVUS</p>
                </Link>
            </aside>
            <nav className="text-white flex justify-center gap-7 w-2/4 flex-col lg:flex-row text-center mt-3 lg:mt-0">
                <Link className="font-bold text-content2 lg:text-content" to="/">SOBRE NOSOTROS</Link>
                <Link className="font-bold text-content2 lg:text-content" to="/productos">PRODUCTOS</Link>
                <Link className="font-bold text-content2 lg:text-content" to="/">TERMINOS</Link>
                <Link className="font-bold text-content2 lg:text-content" to="/">CONTACTO</Link>
            </nav>
            <aside className="text-white flex items-center gap-5 w-full lg:w-1/4 lg:flex-row flex-col mt-3 lg:mt-0">
                <Link className="flex items-center font-bold text-content2 lg:text-content bg-secondary py-2 px-5 rounded" to="/">
            ...
            >
                <form className="h-[500px] pr-10 pl-10">
                    <h2 className="text-h1Login font-bold">Inicia sesión en Cervalia</h2>
                    <p className="text-pLogin mb-5 text-[#545454]">Elige una de las opciones para continuar</p>
                    <TextInput placeholder="Dirección e-mail" required type="email" />
                    <PasswordInput placeholder="Contraseña" required mt="sm" />
                    <div className="my-5 flex items-center">
                        <p className="mx-3 text-pLogin text-[#545454]">O continúa con</p>
                    ...
                    <div className="flex items-center justify-center mb-10">
                        <button
                            ...
                            type="submit"
                            onClick={() => setOpened(false)}  // Cierra el modal de inicio de sesión
                        >
                            Iniciar sesión
                        </button>
                    </div>
                    ...
                    <div className="w-full h-full max-w-[70%] max-h-[90%]  flex justify-center items-center overflow-auto bg-white rounded-3xl p-10 relative">
                        ...
                        <div className="flex flex-raw w-full h-full">
                           <div className="w-1/3 bg-secondary flex items-end justify-center rounded-l-3xl border-r-4 border-[#545454] h-full p-0">
                            ...
                            <img className="w-200 object-contain" src="public/register.png" alt="logo ciervus" />
                            </div>
                            

                            <form className="w-2/3 h-full max-w-[80%] pr-10 pl-10 bg-white p-10 rounded-lg">
                              <h2 className="text-h1Login font-bold text-primary">Regístrate en CIERVUS</h2>
                              <p className="text-pLogin mb-5 text-[#545454]">Añade tus datos para poder configurar tu perfil</p>
                              <div className="flex flex-col gap-5">
                              <TextInput placeholder="Nombre completo" required type="text" />
                              <TextInput placeholder="Nombre de Usuario" required type="text" />
                              <TextInput placeholder="Dirección e-mail" required type="email" />
                              <TextInput placeholder="CIF" required type="number" />
                              <TextInput placeholder="Número de teléfono" required type="tel" />
                              <PasswordInput placeholder="Contraseña" required mt="sm" />
                              <PasswordInput placeholder="Comfirmar contraseña" required mt="sm" />
                              </div>
                              
                              <div className="flex flex-col justify-center items-start gap-10 mt-10 mb-8">
                                <div className="flex flex-row items-center gap-2">
                                <input type="checkbox" name="remember" id="" />
                                <label htmlFor="remember" className="text-pLogin text-[#545454]">
                                    Acepto los{" "}
